...
API_KEY = "72ad6ae2-5e14-40e5-98ce-2960f653ddae"  # Replace with your actual API key
...
with gr.Blocks(css=".gradio-container {background-color: #f0f4f8;}") as demo:
    ...
    with gr.Tab("Character Creation"):
        ...
        sadness = gr.Slider(0, 10, label="Sadness", value=5)
        joy = gr.Slider(0, 10, label="Joy", value=5)
        anger = gr.Slider(0, 10, label="Anger", value=5)
        fear = gr.Slider(0, 10, label="Fear", value=5)
        disgust = gr.Slider(0, 10, label="Disgust", value=5)
        trust = gr.Slider(0, 10, label="Trust", value=5)
        negative = gr.Slider(0, 10, label="Negative", value=5)
        positive = gr.Slider(0, 10, label="Positive", value=5)
        aggressive = gr.Slider(0, 10, label="Aggressive", value=5)
        peaceful = gr.Slider(0, 10, label="Peaceful", value=5)
        cautious = gr.Slider(0, 10, label="Cautious", value=5)
        open_trait = gr.Slider(0, 10, label="Open", value=5)
